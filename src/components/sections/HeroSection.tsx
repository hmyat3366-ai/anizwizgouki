    ...
    <section id="home" className="w-full min-h-[85vh] md:min-h-[90vh] flex flex-col justify-center items-center relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-6 lg:gap-12 xl:gap-16 items-center w-full max-w-[1500px] relative z-10 mx-auto">
        ...
        <div className="justify-self-center lg:justify-self-end flex flex-col items-center lg:items-end text-center lg:text-right mt-8 lg:mt-0 lg:pt-16 reveal hero-parallax w-full relative">
          ...
          <div className="w-full flex justify-center lg:justify-end relative h-[50px] sm:h-[80px] md:h-[100px] lg:h-[70px] xl:h-[90px] 2xl:h-[120px]" dir="rtl">
            <MorphingText
              texts={["UI/UX", "AI PRODUCT"]}
              className="font-display text-[38px] sm:text-[60px] md:text-[72px] lg:text-[50px] xl:text-[62px] 2xl:text-[90px] leading-none font-bold text-foreground uppercase tracking-tighter w-full absolute left-0 top-0 h-full !mx-0 text-center lg:text-right whitespace-nowrap px-2"
            />
          </div>
        ...
        <div className="justify-self-center lg:justify-self-start flex flex-col items-center lg:items-start text-center lg:text-left -mt-6 sm:mt-0 lg:mt-0 lg:pt-16 reveal hero-parallax w-full relative overflow-visible">
          ...
          <div className="w-full flex justify-center lg:justify-start relative h-[50px] sm:h-[80px] md:h-[100px] lg:h-[70px] xl:h-[90px] 2xl:h-[120px]">
            <MorphingText
              texts={["DESIGNER", "DESIGNER"]}
              className="font-display text-[45px] sm:text-[70px] md:text-[85px] lg:text-[60px] xl:text-[75px] 2xl:text-[110px] leading-none font-bold text-foreground uppercase tracking-tighter w-full absolute left-0 top-0 h-full !mx-0 text-center lg:text-left whitespace-nowrap px-2"
            />
          </div>
          <p className="max-w-[280px] text-muted-foreground text-sm sm:text-base mt-6 leading-relaxed font-medium lg:ml-1">
            I turn messy problems into clean, production-ready interfaces — from Figma to code.
          </p>
          <div className="mt-8 flex gap-4 lg:ml-1">
            <button 
              onClick={() => document.getElementById('project')?.scrollIntoView({ behavior: 'smooth' })}
              className="px-6 py-3 rounded-full bg-foreground text-background font-medium text-sm hover:scale-105 transition-transform duration-300 shadow-lg shadow-black/10 dark:shadow-white/10"
            >
              View My Work
            </button>
          </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 pointer-events-none z-10">
        ...
        <div className="w-px h-10 bg-gradient-to-b from-muted-foreground/60 to-transparent animate-bounce" />
      </div>
    </section>
  );
}
