...
export default function CaseStudiesSection({ onOpenStudy }: Props) {
  return (
    <section id="project" className="w-full mt-24 md:mt-40 flex flex-col items-center">
      <div className="w-full flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6 reveal">
        <div className="max-w-xl">
          ...
          <h2 className="font-display text-[50px] md:text-[70px] uppercase font-bold leading-none mb-4 text-foreground">Selected Work</h2>
          <p className="text-muted-foreground text-base leading-relaxed max-w-md mt-4">
          ...
          return (
            <div key={s.id} onClick={() => onOpenStudy(slug)} role="button" tabIndex={0}
              ...
              <div className="w-full h-[300px] lg:h-full relative overflow-hidden order-1 lg:order-2">
                <img src={s.image} loading="lazy" className={`w-full h-full transition-transform duration-700 ease-out group-hover:scale-110 ${s.id === "dmar" ? "object-cover object-top bg-secondary" : "object-cover"}`} />
              </div>
            </div>
          );
        })}
