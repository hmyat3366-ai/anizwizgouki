    ...
    >
      {study && (
        <div className="w-full h-full flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="w-full lg:w-[50vw] h-[65vh] sm:h-[60vh] lg:h-full overflow-y-auto flex flex-col bg-white dark:bg-[#0a0a0a] scroll-smooth">
            {/* Sticky nav bar */}
            ...
            <div className="p-6 sm:p-10 lg:p-12 xl:p-16 flex flex-col flex-grow">
              ...
              <div className="mb-12 lg:mb-16">
                <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl xl:text-[68px] font-bold leading-[0.92] text-foreground mb-3 tracking-[-0.03em]">{study.title} —</h1>
                {study.subtitle && (
                  <p className="text-base sm:text-lg font-semibold text-muted-foreground mb-4 tracking-[-0.01em]">{study.subtitle}</p>
                )}
                <p className="text-lg sm:text-xl lg:text-2xl font-normal leading-[1.4] tracking-[-0.01em] text-muted-foreground max-w-xl mt-4">{study.desc}</p>

                <div className="grid grid-cols-2 gap-8 mt-10 max-w-lg border-l-2 border-gray-900 dark:border-white pl-6">
                  <div>
                    <h4 className="text-[10px] font-bold tracking-[0.15em] uppercase text-muted-foreground mb-2">
                      {isXiaChat ? "Duration" : "Timeline / Deadline"}
                    </h4>
                    <p className="text-sm font-bold text-foreground">{study.timeline}</p>
                    {study.timelineNote && (
                      <p className="text-[12px] text-muted-foreground mt-1 leading-snug">{study.timelineNote}</p>
                    )}
                  </div>
                  <div>
                    <h4 className="text-[10px] font-bold tracking-[0.15em] uppercase text-muted-foreground mb-2">Tools</h4>
                    <p className="text-sm font-bold text-foreground leading-relaxed">{study.tools.join(" • ")}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-10 lg:gap-12 mb-12 lg:mb-14">
                <div>
                  <h3 className="text-[11px] font-bold tracking-[0.15em] uppercase text-muted-foreground mb-6">Role —</h3>
                ...
                <div>
                  <h3 className="text-[11px] font-bold tracking-[0.15em] uppercase text-muted-foreground mb-6">Project Overview —</h3>
                  {study.overview.split("\n\n").map((para, i) => (
                    <p key={i} className="text-[15px] text-gray-600 dark:text-gray-400 leading-[1.7] mb-4 last:mb-0">{para}</p>
                  ))}
                </div>
              </div>
              ...
              <div className="mb-12 lg:mb-14">
                <h3 className="text-[11px] font-bold tracking-[0.15em] uppercase text-muted-foreground mb-6">
              ...
              {study.keyFeatures && study.keyFeatures.length > 0 && (
                <div className="mb-12 lg:mb-14">
                  <h3 className="text-[11px] font-bold tracking-[0.15em] uppercase text-muted-foreground mb-6">Key Features —</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-2 gap-3">
                    {study.keyFeatures.map((feat, i) => (
                      <div key={i} className="flex items-center gap-3 py-3 px-4 border border-border/60 bg-gray-50 dark:bg-gray-900/40 rounded-sm group hover:border-primary/40 transition-colors duration-200">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#06B6D4] shrink-0" />
                        <span className="text-[13px] text-gray-700 dark:text-gray-300 font-medium">{feat}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className={`p-6 sm:p-8 border border-border mb-12 lg:mb-14 ${isXiaChat ? "bg-gray-50 dark:bg-gray-900/50" : "bg-gray-50 dark:bg-gray-900/50"}`}>
                <span className="text-[10px] font-bold tracking-[0.15em] uppercase text-muted-foreground block mb-3">
              ...
              {/* ── Bottom CTA ── */}
              <div className="mt-auto pt-8 border-t border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
